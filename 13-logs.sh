...
if [ $? -ne 0 ]
then
    echo "httpd is not installed......going to install" | tee -a $LOG_FILE
...
VALIDATE $? "mysql"
    ...
    echo -e "mysql is installed... $Y already installed $N" | tee -a $LOG_FILE
fi
...
VALIDATE $? "python3"
    ...
    echo -e "python3 is installed... $Y already installed $N" | tee -a $LOG_FILE
fi